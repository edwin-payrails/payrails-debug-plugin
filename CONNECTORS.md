...
**Caveats**: the Cortex Analyst tools only *generate* SQL — run it via `execute-sql` to get rows. Snowflake data lags real time (use it for patterns/trends, not "this payment right now"). For **Cowork**, no extra setup is needed — the GitHub-installed plugin's `.mcp.json` carries Snowflake into Cowork too (verified 2026-06-25); see the **Cowork note** under "Setup reminders" below. (For the `ANALYST` access request itself, see the [Snowflake MCP Access](https://app.notion.com/p/384cc40840c181dd8facc51ca24325ec) Notion guide.)

...
10. **Grafana** (via the `gcx` CLI) — Run `gcx` in the shell for transaction-level investigation (logs/traces for specific execution or payment IDs), performance and health investigation (rates, latency, trends via Prometheus metrics), trace inspection (Tempo), and dashboard exploration. Looking up specific execution or payment IDs in Loki logs is the most common use during debugging sessions. **Read `references/grafana.md`** for the gcx command patterns, the real datasource UIDs, the namespace ladder, interpretation patterns, and gotchas. *(There is no Grafana MCP — drive everything through `gcx`; a dormant `grafana` MCP may appear in config, ignore it.)*
...
- **For logs or traces of a specific execution or payment ID:** use the `gcx` CLI, scoped to the correct environment and merchant (see the namespace ladder in `references/grafana.md`). For broader Grafana-based investigation (PSP health as a last-resort hypothesis, metric-based performance checks, dashboard exploration, resolving a shared `/goto` link), see `references/grafana.md` for the full set of patterns. When only transaction-level lookup is needed, Loki log queries via `gcx logs query` are the direct path.

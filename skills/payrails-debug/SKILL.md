...
10. **Grafana** (via the **Grafana MCP**) — use the `grafana` MCP tools (`query_loki_logs`, `query_prometheus`, `search_dashboards`, the `tempo_*` trace tools, etc.) for transaction-level investigation (logs/traces for specific execution or payment IDs), performance and health investigation (rates, latency, trends via Prometheus metrics), trace inspection (Tempo), and dashboard exploration. Looking up specific execution or payment IDs in Loki logs is the most common use during debugging sessions. **Read `references/grafana.md`** for the MCP tool parameters, the real datasource UIDs, the namespace ladder, interpretation patterns, and gotchas. *(The Grafana MCP authenticates via browser OAuth on first use — see grafana.md; it needs the Grafana "Assistant" role, admin-granted.)*

   **Critical: Always scope to the correct environment and merchant before querying Grafana.** Staging and production use different namespaces, and which label identifies a merchant depends on its deployment — see the namespace ladder in `references/grafana.md`. If the SE has not specified the environment, ask first. Never query without correct scoping — results from the wrong environment mislead and waste time.
...
13. **Snowflake** (via the `snowflake` MCP server) — the source of truth for **historical and aggregate** payment data: authorization/decline-rate trends, decline-reason analysis, provider/BIN/routing performance, fees/reconciliation, volumes, retry-recovery, impacted-transaction sets, and payment anomalies — sliced by merchant, provider, method, country, or metadata field. **Read `references/snowflake.md`** for tool selection and the rule that the Cortex Analyst tools only *generate* SQL (run it via `execute-sql` to get rows). Reach for Snowflake when the question is "what's the pattern across X over time"; use Grafana/Temporal/the API for a single payment or execution. Note: Snowflake data lags real-time.
...
3. **Grafana** (if the SE has provided an execution ID or payment ID) — query Loki logs for the specific ID via the **Grafana MCP** (`query_loki_logs`), scoped to the correct environment and merchant (see the namespace ladder in `references/grafana.md`). See `references/grafana.md` for query details and the wider set of Grafana uses during debugging. Transaction-level log lookup often gives you the answer immediately.
...
- **For logs or traces of a specific execution or payment ID:** use the **Grafana MCP** (`query_loki_logs` / the `tempo_*` tools), scoped to the correct environment and merchant (see the namespace ladder in `references/grafana.md`). For broader Grafana-based investigation (PSP health as a last-resort hypothesis, metric-based performance checks, dashboard exploration), see `references/grafana.md` for the full set of patterns. When only transaction-level lookup is needed, Loki log queries (`query_loki_logs`) are the direct path.
...
- **For aggregate or trend data — "is this isolated or systemic?":** once you've looked at the specific failing payment, use the `snowflake` MCP to check whether it's part of a wider pattern (auth-rate or decline-reason movement over the window, the impacted-transaction set, provider/BIN trends). See `references/snowflake.md`. Snowflake is for the pattern across many payments; Grafana/Temporal/the API are for the single one. (Snowflake data lags real-time, so don't use it for "right now".)
